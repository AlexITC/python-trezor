...
class StakenetTxApi:
    def __init__(self, coin_data):
        ...
        if coin_data["custom_explorer"]["urls"]:
            self.url = random.choice(coin_data["custom_explorer"]["urls"])
    ...
    def fetch_json(self, *path, **params):
        ...
        return requests.get(url, params=params, timeout=1).json(parse_float=Decimal)

    def get_block_hash(self, block_number):
        j = self.fetch_json("blocks", block_number, "raw")
        return bytes.fromhex(j["hash"])

    def current_height(self):
        j = self.fetch_json("stats")
        return j["blocks"]
    ...
    def get_tx_data(self, txhash):
        data = self.fetch_json("transactions", txhash, "raw")
        return data
